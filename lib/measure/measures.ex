...
defmodule Bonfire.Quantify.Measures do
  ...
  * Various parts of the codebase that need to query for collections (inc. tests)
  """
  def many(filters \\ []),
    do: {:ok, repo().many(Queries.query(Measure, filters))}

  ## mutations

  @spec create(any(), Unit.t(), attrs :: map) ::
          {:ok, Measure.t()} | {:error, Changeset.t()}
  ...
  defp insert_measure(creator, unit, attrs) do
    ...
    repo().insert(
      Bonfire.Quantify.Measure.create_changeset(creator, unit, attrs)
  ...
  @spec update(Measure.t(), attrs :: map) ::
          {:ok, Measure.t()} | {:error, Changeset.t()}
  def update(%Measure{} = measure, attrs) do
    repo().transact_with(fn ->
      with {:ok, measure} <-
             repo().update(Measure.update_changeset(measure, attrs)) do
